...
- **Quick create**: Title field (auto-capitalized), date picker (default: tomorrow), probability slider with quick-set chips (10/25/50/75/90%) and −/+ bits-of-evidence steppers flanking the big percent readout (each tap is 1 bit of log-odds: + doubles the odds, − halves them; pure math in `BitsMath`, clamped to 1–99%; part of the shared `ProbabilitySlider`, so they also appear in the detail sheet's binary and per-option forecast updates), free-text tag chips (sent with createQuestion). Entry points: FAB, ACTION_SEND share target (shared text prefills the title via the `create?prefill=` route arg + SavedStateHandle), and static launcher shortcuts ("New prediction", "Ready to resolve")
